...
<template>
  <button 
    ...
    class="flex items-center justify-center gap-2 px-6 py-3 bg-linear-to-r bg-green-600 text-white rounded-lg font-semibold hover:bg-green-700 transition-all transform hover:-translate-y-1 hover:shadow-lg"
  >
    <span>Add {{ opponent.nickname }}</span>
  </button>
  ...
  <div v-else class="flex items-center justify-center gap-2 px-6 py-3 bg-green-100 text-green-700 rounded-lg font-semibold border-2 border-green-300">
    ...
    <span>{{ opponent.nickname }} Already added on the list</span>
  </div>
</template>